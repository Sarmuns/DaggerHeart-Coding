# Core Design Rules & Dark Mode Protocol

> Source: taste-skill (Leon), Section 4 (Design Engineering Directives) and Section 8 (Dark Mode Protocol). Tailwind class names below are shorthand for the underlying CSS; translate them to the project's actual stack when Tailwind is not in use.
